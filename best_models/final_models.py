...
def buildConcatModelTitleMeta(model_bert_title, model_meta, NUM_CLASS, isPreTrained=False, pathToModelWeights=None, isTrainable=True):
    ...
    x = Dropout(0.4)(x)
    output = Dense(NUM_CLASS, activation='softmax', name='final_multimodal_output')(x)
    model = Model([model_bert_title.input, model_meta.input], output)
    if not isPreTrained:
        return model             
    else:
        model.load_weights(pathToModelWeights)
